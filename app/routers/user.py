...
Secret = ("6217bb23e89b7030aa36362a7111c9f2efc620c1efeb8f97dda9b5c125fb2f934359a808cb55fc98a7a370742abcaaacc59768"
          "a9df41b2fddc1b3dffdbd5361f")
Algorithm = "HS256"
...
async def get_user(token: Annotated[str, Depends(bearer)]):
    try:
        ...
        print(f'JWTError occurred as : {e}')
...
@user.post("/signup", status_code=status.HTTP_201_CREATED,
           response_description={201: {'description': 'User has signed up successfully.'}})
...
async def log_in(form: Annotated[OAuth2PasswordRequestForm, Depends()], db: db_dependency):
    ...
    if not token:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Error while trying to log you in please try again later!")
...
# Get user details
@user.get("/user-details", status_code=status.HTTP_200_OK,
          response_description={200: {'description': 'User has requested for the details.'}})
async def get_current_user(user: user_dependency, db: db_dependency):
    ...
    if not details:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="An error occurred while trying to fetch details, please try again later")
    data = GetUserDetails(
...
# User update details
@user.put("/user-details/update", status_code=status.HTTP_202_ACCEPTED,
          response_description={202: {'description': 'User has successfully change the details'}})
async def update_details(user: user_dependency, db: db_dependency, form: UpdateDetails):
    ...
    if not details:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="User not found, please login and try again later!")
...
@user.put("/forgot-password/recovery", status_code=status.HTTP_202_ACCEPTED,
          response_description={202: {'description': 'User has successfully verified account and changed password'}})
...
@user.put("/change-password/", status_code=status.HTTP_202_ACCEPTED,
...
# User delete
@user.delete("/user/delete-user", status_code=status.HTTP_204_NO_CONTENT,
             response_description={204: {'description': 'User has decided to delete his account'}})
async def delete_user(user: user_dependency, db: db_dependency):
    ...
    if not user_delete:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="We encountered an error while trying to delete your data, please try again later.")
